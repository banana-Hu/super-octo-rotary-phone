...
三张图片均返回 `success`，逐人 PNG 和主体 PNG 均正常写入。最终使用笔记本图片执行 `momentmaker-check --subject-mode foreground`，返回 `ready`、无产物错误，并得到 `primary_subject_id: 1`。首次包含模型初始化的单张总耗时约 19–32 秒；后续耗时取决于图片尺寸和设备。比赛前必须预下载权重，并以实际演示机复测。

## 结论

当前实现通过 72 项单元测试，满足黑客松 MVP：保留原有逐人结果，近距离人物可合并，典型手持或共同使用物体可随主体输出，并具备失败降级。它不是通用关系理解模型；机器人等未形成清晰连接的物体仍应列为已知限制。
